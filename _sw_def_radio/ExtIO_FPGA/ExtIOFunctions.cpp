/*
	...
	29.10.2024	-	Adopt for Marsohod3GW2
...
#define FIXED_BIT 32
uint32_t float2fix(double n)
{
	uint32_t int_part = 0, frac_part = 0;
	int i;

	int_part = (uint32_t)floor(n) << FIXED_BIT;
	n -= (uint32_t)floor(n);
...
void send_freq(int frequency)
{
	...
	double angle_incr = double(frequency) / double(OSC_FREQ);
	uint32_t angle_incr_fp = float2fix(angle_incr);

	char fp_str[33];
	fp_str[32] = 0;
	uint32_t a = angle_incr_fp;
	for (int i = 0; i < 32; i++) {
		fp_str[i] = 0x30 + ((a & 0x80000000) ? 1 : 0 );
		a = a << 1;
	}
	printf("Set Freq %d Angle Incr %f %08X Angle Incr Fp %s\n", frequency, angle_incr, angle_incr_fp, fp_str);

	pkt[0] = 0x80;
	pkt[1] = (angle_incr_fp >> 0 ) & 0xFF;
	pkt[2] = (angle_incr_fp >> 8 ) & 0xFF;
	pkt[3] = (angle_incr_fp >> 16) & 0xFF;
	pkt[4] = (angle_incr_fp >> 24) & 0xFF;
...
extern "C" bool __stdcall InitHW(char *name, char *model, int& type)
{
	...
	char* my_sdr_name  = "SDR M3GW2 FPGA";
	char* my_sdr_model = "SDR M3GW2 v1.0";
